...
trait ApiResponser
{
    ...
    {
        if (!($instance instanceof Model || $instance instanceof Collection || is_array($instance))) {
            throw new \InvalidArgumentException('El argumento $instance debe ser de tipo Model, Collection o array');
        }
    ...
    protected function showOne(Model|array|Collection $instance, ?Collection $audit = null, $code = 200)
    {
        ...
        if ($audit !== null) {
            $payload['audits'] = $audit;
        }

        return $this->successResponse($payload, $code);
    }
    ...
    protected function cacheResponse($data)
    {
        ...
        $fullUrl = "{$url}?{$queryString}|user:{$userId}";
