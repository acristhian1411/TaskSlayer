...
.PHONY: help up down build rebuild ps logs-backend logs-frontend bash-backend sh-backend composer artisan migrate key fix-perms

help:
	@echo "Available commands:"
	@echo "  make up                Start backend and postgres"
	@echo "  make down              Stop containers"
	@echo "  make build             Build images"
	@echo "  make rebuild           Rebuild and restart backend + postgres"
...
up:
	$(DOCKER_COMPOSE) up -d backend postgres
...
rebuild:
	$(DOCKER_COMPOSE) up -d --build backend postgres
